...
export async function getSuggestedPathsByOrganization(
  organizationId: string
): Promise<{ paths: SuggestedLearningPath[]; generatedAt: Date | null }> {
...
export async function upsertSuggestedPathsForOrganization(
  organizationId: string,
  paths: SuggestedLearningPath[]
): Promise<{ paths: SuggestedLearningPath[]; generatedAt: Date }> {
  const collection = await getOnboardingPathSuggestionsCollection();
  const now = new Date();
