/**
 * Fetches real context from a GitHub repo (README excerpt, root layout) so the LLM
 * can suggest onboarding paths grounded in what the repo actually contains.
 */
...
export async function fetchRepoContext(
  ...
  try {
    const readmeRes = await fetch(
      `https://api.github.com/repos/${repoFullName}/readme?ref=${branch}`,
      { headers }
    );
    if (readmeRes.ok) {
