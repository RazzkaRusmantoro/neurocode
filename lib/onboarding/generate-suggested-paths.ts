...
function formatRepoBlock(
  repo: { name: string; description?: string; readmeExcerpt?: string | null; rootEntries?: string[] }
): string {
  const lines: string[] = [`## Repository: ${repo.name}`];
  if (repo.description) lines.push(`Description (from GitHub): ${repo.description}`);
  if (repo.rootEntries?.length) lines.push(`Root-level entries: ${repo.rootEntries.join(', ')}`);
...
const USER_PROMPT_TEMPLATE = (
  orgName: string,
  repos: { name: string; description?: string; readmeExcerpt?: string | null; rootEntries?: string[] }[]
) => {
...
interface LLMPathItem {
  ...
  modules: { name: string; summaryDescription: string; order: number }[];
}
...
function toSuggestedPaths(llmPaths: LLMPathItem[]): SuggestedLearningPath[] {
  return llmPaths.map((p) => {
    ...
    return {
      ...
      modules: modules.length >= 7 ? modules : modules, // allow 7-10; if LLM gave fewer we keep what we have
    };
  });
}
...
export async function generateSuggestedPaths(input: OrgSummaryInput): Promise<SuggestedLearningPath[]> {
  ...
  // Ensure each path has 7-10 modules; if LLM gave fewer, we still return (client can enforce later)
  return paths;
}
