...
function BentoCard({ item }: { item: typeof bentoItems[0] }) {
  ...
  function handleMouseMove(e: MouseEvent) {
    ...
    const xPct = (mouseXPos / width - 0.5) * 2; // -1 to 1
    const yPct = (mouseYPos / height - 0.5) * 2; // -1 to 1
  ...
  return (
    <motion.div
      ...
      className={`relative group rounded-[2rem] border border-white/5 bg-[#121214] p-8 md:p-10 flex flex-col justify-between overflow-hidden ${item.colSpan}`}
    >
      ...
      <div 
        className="relative z-10 transform-gpu transition-transform duration-500 group-hover:translate-z-10"
      ...
      {/* Ambient static gradient for depth */}
...
export default function InteractiveBento() {
  return (
    <section className="py-24 md:py-32 relative bg-[#0a0a0b] overflow-hidden [perspective:1000px]">
