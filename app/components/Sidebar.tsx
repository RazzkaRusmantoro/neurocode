  ...
  if (hasSubItems) {
    ...
    return (
      <div>
        <button 
          onClick={onToggle}
          className="w-full text-left pl-4 pr-4 py-3 text-sm hover:bg-[#2a2a2a]/50 rounded-lg transition-colors duration-200 flex items-center justify-between gap-3 cursor-pointer"
        >
          <div className="flex items-center gap-3">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.iconPath} />
            </svg>
            <span className={parentIsActive ? 'text-white font-bold' : 'text-white/60 hover:text-white'}>{item.label}</span>
          </div>
          <svg className={`w-4 h-4 text-white/60 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
