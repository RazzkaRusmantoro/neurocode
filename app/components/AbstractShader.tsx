...
export const AbstractShader = memo(function AbstractShader({
  className = "",
}: {
  className?: string;
}) {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const animate = (time: number) => {
        ...
        if (sceneRef.current) sceneRef.current.rafId = rafId;
    ...
    return () => {
      ...
      if (ref) {
        ...
        if (container.contains(ref.canvas)) container.removeChild(ref.canvas);
