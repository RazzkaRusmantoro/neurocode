...
// ── Extracted style constants (avoid new objects per render) ──
...
const CountUpStep = memo(function CountUpStep({
  number,
  visible,
  position = "bottom",
}: {
  ...
  return (
    <motion.div
      className="relative flex min-h-[6rem] w-full shrink-0 justify-center pt-2"
      initial={false}
      animate={visible ? { y: 0, opacity: 1 } : { y: hiddenY, opacity: 0 }}
      transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
    >
      <span
        className="text-6xl font-semibold tabular-nums opacity-50 md:text-7xl lg:text-8xl"
        style={COUNTUP_STYLE}
      >
        {label}
      </span>
...
/** Glossary & Code Reference: cursor-tracked spiral, orange, 50% visibility.
 *  Uses rAF + ref for mask position (no React re-renders on mouse move). */
const GlossaryCardPattern = memo(function GlossaryCardPattern({
  positionRef,
}: {
  positionRef: { readonly current: { x: number; y: number } };
}) {
  ...
  return (
    <div
      ref={maskRef}
      className="pointer-events-none absolute inset-0 z-0 flex items-center justify-center overflow-hidden rounded-2xl"
    >
      <SpiralAnimation
        totalDots={600}
        size={400}
...
export function HorizontalAccordion({ items }: { items: AccordionItem[] }) {
  ...
  // Visible when: in view, or scrolled down past it. Hidden when scrolled back up past it.
  useEffect(() => {
    ...
    const update = () => {
      ...
      setAccordionVisible((prev) => {
        ...
        if (aboveViewport) return scrollingDown;
        if (belowViewport) return false;
        return prev;
      });
    };
    update();
  ...
  // Ref-based mouse positions (no React state → no re-renders on move)
  ...
  // Stable ref callbacks for content panels
  ...
  // ── Handlers (useCallback = stable references) ──
  ...
  const handleGlossaryRefsMouseMoveFromContent = useCallback(
    (e: React.MouseEvent) => {
  ...
  const handleUmlMouseMoveFromContent = useCallback(
    (e: React.MouseEvent) => {
      const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
      umlMouseRef.current = {
        x: e.clientX - rect.left,
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () => setChevronCenteredIndex(hoveredIndex),
      CHEVRON_MOVE_TO_CENTER_DELAY_MS,
    );
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (!liveDocsExpandedRef.current) return;
  ...
  // Set initial Glossary gradient position on expand
  useEffect(() => {
    const isGlossaryExpanded =
      hoveredIndex !== null && items[hoveredIndex]?.id === "glossary-refs";
    if (!isGlossaryExpanded) return;
    const raf = requestAnimationFrame(() => {
      const el = glossaryRefsContentRef.current;
      if (!el) return;
      const rect = el.getBoundingClientRect();
  ...
  return (
    <div ref={ref} className="relative flex justify-center w-full">
      {/* Ambient orange glow (timeline-style) */}
      ...
      <motion.div
        className="relative z-10 inline-flex h-[28rem] min-h-[420px] items-stretch gap-12 md:h-[32rem]"
        initial="hidden"
        animate={accordionVisible ? "visible" : "hidden"}
      ...
      >
        {items.map((item, index) => {
          ...
          const numberBlock = (
            <div className="min-h-[6rem] w-full overflow-hidden">
          ...
          return (
            <div key={item.id} className="flex flex-col items-center shrink-0">
              ...
              <motion.div
                className={`relative h-full shrink-0 rounded-none border border-white/10 ${numberOnTop ? "mt-5" : ""}`}
                variants={{
              ...
              >
                {/* Orange outline: 4 edges drawn clockwise on hover, reverse on leave */}
                <motion.div
                  className="pointer-events-none absolute top-0 left-0 z-[999] h-px w-full bg-orange-500"
                  style={EDGE_TOP_STYLE}
                  initial={false}
                  animate={{ scaleX: isExpanded ? 1 : 0 }}
                  transition={{ duration: EDGE_DURATION, delay: isExpanded ? 0 : EDGE_DURATION * 3, ease: "linear" }}
                />
                <motion.div
                  className="pointer-events-none absolute top-0 right-0 z-[999] h-full w-px bg-orange-500"
                  style={EDGE_RIGHT_STYLE}
                  initial={false}
                  animate={{ scaleY: isExpanded ? 1 : 0 }}
                  transition={{ duration: EDGE_DURATION, delay: isExpanded ? EDGE_DURATION : EDGE_DURATION * 2, ease: "linear" }}
                />
                <motion.div
                  className="pointer-events-none absolute bottom-0 left-0 z-[999] h-px w-full bg-orange-500"
                  style={EDGE_BOTTOM_STYLE}
                  initial={false}
                  animate={{ scaleX: isExpanded ? 1 : 0 }}
                  transition={{ duration: EDGE_DURATION, delay: isExpanded ? EDGE_DURATION * 2 : EDGE_DURATION, ease: "linear" }}
                />
                <motion.div
                  className="pointer-events-none absolute top-0 left-0 z-[999] h-full w-px bg-orange-500"
                  style={EDGE_LEFT_STYLE}
                  initial={false}
                  animate={{ scaleY: isExpanded ? 1 : 0 }}
                  transition={{ duration: EDGE_DURATION, delay: isExpanded ? EDGE_DURATION * 3 : 0, ease: "linear" }}
                />

                {/* Inner wrapper: clips content overflow without affecting borders */}
                <div className="relative flex h-full w-full overflow-hidden bg-[#121214]">
                  {/* Gray to dark orange gradient at 10% opacity */}
                  <div
                    className="pointer-events-none absolute inset-0 z-0 opacity-[0.1]"
                    style={GRADIENT_BG_STYLE}
                  />

                  {/* Content: fixed width so text layout never changes during expand/collapse */}
                  <div className="relative z-10 flex h-full w-[460px] shrink-0 overflow-hidden rounded-none bg-transparent">
                    <div className="relative flex h-full w-[128px] shrink-0 flex-col items-stretch overflow-hidden bg-[#0d0d0f]">
                      {/* Strip title: slides left out of view when hovered */}
                      <motion.div
                        className="flex flex-1 items-center justify-center px-2 pt-2 pb-14"
                        style={STRIP_TITLE_STYLE}
                        initial={false}
                      ...
                      <motion.span
                        className={`absolute left-1/2 flex -translate-x-1/2 shrink-0 ${isExpanded ? "text-orange-400" : "text-white/50"}`}
                        style={{
                    ...
                    <motion.div
                      ...
                      className="relative flex w-[332px] shrink-0 flex-col overflow-hidden"
                      initial={false}
                    ...
                    >
                      ...
                      />
                      {/* Description: centered. Title appears from bottom-right, rises up */}
                      <div className="relative z-20 flex min-h-0 flex-1 flex-col items-center justify-center overflow-hidden px-8 pt-8 pb-20 text-center">
                        <p
                          className="max-w-md text-base font-light leading-relaxed text-[#a1a1aa] md:text-lg"
                          style={RED_HAT_DISPLAY_STYLE}
                        >
                          {item.description}
                        </p>
                        <motion.h3
                          className="absolute bottom-6 right-6 text-right text-2xl font-extralight uppercase tracking-wide text-white md:text-3xl"
                          style={RED_HAT_MONO_STYLE}
                          initial={false}
                          animate={{
                            y: isExpanded ? 0 : 28,
