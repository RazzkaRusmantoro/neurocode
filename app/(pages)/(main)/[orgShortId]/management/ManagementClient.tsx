'use client';
import { Copy, Link2, Send, Search, MoreHorizontal, } from 'lucide-react';
        ...
        <div className="pb-6 mb-6 border-b border-[#262626]">
          ...
          <div className="flex flex-wrap gap-4 items-stretch">
            ...
            <div className="flex-1 min-w-0 flex items-center gap-2 bg-[#1a1a1a] border border-[#262626] rounded-lg overflow-hidden px-2 py-1.5 min-h-0">
              ...
              <input type="text" value={inviteLink} onChange={(e) => setInviteLink(e.target.value)} placeholder="Invite link" className="flex-1 min-w-0 bg-transparent px-1 py-1.5 text-white text-sm placeholder-white/40 focus:outline-none"/>
              <button type="button" onClick={handleCopyLink} className="shrink-0 px-3 py-1.5 rounded-md text-sm font-medium text-white/80 hover:text-white hover:bg-[#333333] transition-all duration-200 flex items-center gap-1.5 cursor-pointer border border-[#404040] origin-right hover:scale-x-105">
            ...
            <div className="flex-1 min-w-0 flex items-center gap-2 bg-[#1a1a1a] border border-[#262626] rounded-lg overflow-hidden px-2 py-1.5 min-h-0">
              <input type="email" placeholder="Email address" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} className="flex-1 min-w-0 bg-transparent px-1 py-1.5 text-white text-sm placeholder-white/40 focus:outline-none"/>
              <button type="button" className="shrink-0 px-3 py-1.5 rounded-md text-sm font-medium text-white transition-all duration-200 cursor-pointer border-0 origin-right hover:scale-x-105 flex items-center gap-1.5" style={{ background: ACCENT }}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#262626]">
                  <th className="w-12 py-4 pl-6 pr-2 text-left">
                    <Checkbox checked={filteredMembers.length > 0 && filteredMembers.every((m) => selectedMemberIds.has(m.id))} onCheckedChange={toggleSelectAll} aria-label="Select all"/>
                  </th>
              ...
              <tbody className="divide-y divide-[#262626]">
                {filteredMembers.map((member) => (<tr key={member.id} className="hover:bg-[#121215]/50 transition-colors">
                    <td className="py-4 pl-6 pr-2 w-12">
                      <Checkbox checked={selectedMemberIds.has(member.id)} onCheckedChange={() => toggleMember(member.id)} aria-label={`Select ${member.name}`}/>
                    </td>
                    ...
                    <td className="py-4 px-6 w-12">
                      <button type="button" className="p-2 rounded-lg text-white/50 hover:text-white hover:bg-[#2a2a2a] transition-colors" aria-label="Manage member">
