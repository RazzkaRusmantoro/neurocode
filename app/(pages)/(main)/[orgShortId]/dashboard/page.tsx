...
import { getTaskCompassResultsForAssignedTasks, } from '@/lib/task_compass_storage';
...
function ImpactChartTooltip({ active, payload, }: {
    active?: boolean;
    payload?: Array<{
        payload: (typeof IMPACT_DATA)[0];
    }>;
}) {
    if (!active || !payload?.length)
        return null;
    const p = payload[0].payload;
    return (<div className="bg-[#212121] border border-[#424242] rounded p-3 shadow-lg">
...
function ActivityBadge({ level }: {
    level: 'Stable' | 'Moderate Activity' | 'High Activity';
}) {
    const styles = level === 'High Activity'
    ...
    return (<span className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-medium border ${styles}`}>
      {level}
    </span>);
}
function CautionBadge({ label }: {
    label: string;
}) {
    const isManual = label === 'manual approval';
    return (<span className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-medium border ${isManual ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-amber-500/20 text-amber-400 border-amber-500/30'}`}>
      {label}
    </span>);
}
export default function DashboardPage() {
    ...
    const [assignedTaskResults, setAssignedTaskResults] = useState<ReturnType<typeof getTaskCompassResultsForAssignedTasks>>([]);
    ...
    useEffect(() => {
        ...
        fetch(`${base}/api/organizations/${orgSegment}/onboarding/generated-paths`, { credentials: 'include' })
            ...
            return fetch(`${base}/api/organizations/${orgSegment}/onboarding/suggested-paths`, { credentials: 'include' })
                ...
                .then((suggested) => {
                const paths: OnboardingPathItem[] = (suggested.paths ?? []).filter((p: OnboardingPathItem) => pathIds.includes(p.id));
    ...
    const allCautionAreas = assignedTaskResults.flatMap((r) => (r.cautionAreas ?? []).map((c) => ({ ...c, _taskTitle: r.taskTitle })));
    const seenCautionFiles = new Set<string>();
    const cautionAreasDeduped = allCautionAreas.filter((c) => {
        if (seenCautionFiles.has(c.file))
            return false;
        seenCautionFiles.add(c.file);
        return true;
    });
    const hasAlerts = cautionAreasDeduped.length > 0;
    const totalRelevantFiles = assignedTaskResults.reduce((sum, r) => sum + (r.relevantFiles?.length ?? 0), 0);
    const activityLevel: 'Stable' | 'Moderate Activity' | 'High Activity' = totalRelevantFiles >= 6 ? 'High Activity' : totalRelevantFiles >= 1 ? 'Moderate Activity' : 'Stable';
    const TOP_FILES_PER_TASK = 5;
    return (<div className="mx-auto max-w-screen-2xl pb-24">
          ...
          <div className="w-full h-[240px] -mb-2">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={IMPACT_DATA} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="day" stroke="#666" fontSize={10} tick={{ fill: '#666' }} interval="preserveStartEnd"/>
                <YAxis stroke="#666" fontSize={10} tick={{ fill: '#666' }} tickFormatter={(v) => String(v)} width={32} tickMargin={5}/>
                <Tooltip content={<ImpactChartTooltip />}/>
                <Line type="monotone" dataKey="score" stroke={ACCENT} strokeWidth={2} dot={{ fill: ACCENT, r: 3, strokeWidth: 2, stroke: '#121215' }} activeDot={{ r: 5, fill: ACCENT }}/>
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <section className="mb-8">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          ...
          {onboardingPaths.length > 0 && (<button type="button" onClick={() => router.push(`/${orgSegment}/onboarding`)} className="shrink-0 py-2 px-4 rounded text-sm font-medium bg-[var(--color-primary)] hover:bg-[var(--color-primary-light)] text-white transition-colors">
              View all in Onboarding
            </button>)}
        </div>
        <div className="bg-[#262626]/50 border border-[#262626] rounded overflow-hidden">
          {onboardingLoading ? (<div className="py-10 px-6 text-center text-sm text-white/50">Loading…</div>) : onboardingPaths.length === 0 ? (<div className="py-10 px-6 text-center">
              <p className="text-sm text-white/50 mb-4">No onboarding paths available yet.</p>
              <button type="button" onClick={() => router.push(`/${orgSegment}/onboarding`)} className="py-2 px-4 rounded text-sm font-medium bg-[var(--color-primary)] hover:bg-[var(--color-primary-light)] text-white transition-colors">
                Go to Onboarding
              </button>
                ...
                return (<>
                    ...
                    {inProgress.length > 0 && (<div>
                        ...
                        <div className="space-y-2">
                          {inProgress.slice(0, 2).map((path) => (<div key={path.id} className="flex flex-wrap items-center justify-between gap-3 p-4 rounded-lg bg-[#1a1a1a] border border-[#333]">
                              ...
                              <button type="button" onClick={() => router.push(`/${orgSegment}/onboarding/${slugify(path.title)}`)} className="shrink-0 py-2 px-4 rounded text-sm font-medium bg-[var(--color-primary)] hover:bg-[var(--color-primary-light)] text-white transition-colors">
                                Continue
                              </button>
                            </div>))}
                        </div>
                    ...
                    {notStarted.length > 0 && (inProgress.length === 0 || notStarted.length > 0) && (<div>
                        <h4 className="text-sm font-semibold text-white/90 mb-2">
                          {inProgress.length > 0 ? 'Start next' : 'Recommended'}
                        </h4>
                        <div className="space-y-2">
                          {notStarted.slice(0, 2).map((path) => (<div key={path.id} className="flex flex-wrap items-center justify-between gap-3 p-4 rounded-lg bg-[#1a1a1a] border border-[#333]">
                              <div className="min-w-0 flex-1">
                                <p className="font-medium text-white truncate">{path.title}</p>
                                <p className="text-xs text-white/50 mt-0.5 line-clamp-1">{path.summaryDescription}</p>
                              </div>
                              <button type="button" onClick={() => router.push(`/${orgSegment}/onboarding/${slugify(path.title)}`)} className="shrink-0 py-2 px-4 rounded text-sm font-medium border border-[var(--color-primary)] text-[var(--color-primary)] hover:bg-[var(--color-primary)]/10 transition-colors">
                                Start
                              </button>
                            </div>))}
                        </div>
                    ...
                    {completedCount === totalCount && totalCount > 0 && (<p className="text-sm text-white/50">You’ve completed all available paths.</p>)}
                  </>);
            })()}
            </div>)}
        </div>
      ...
      {hasAlerts && (<section className="mb-8">
          <h3 className="text-lg font-semibold text-white mb-4">Needs Your Attention</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {cautionAreasDeduped.map((item, idx) => (<div key={idx} className="bg-[#262626]/50 border border-[#262626] rounded p-6 flex flex-col">
                ...
                <button type="button" className="mt-auto w-fit py-2 px-4 rounded text-sm font-medium bg-[var(--color-primary)] hover:bg-[var(--color-primary-light)] text-white transition-colors" onClick={() => router.push(`/${orgSegment}/task-compass`)}>
      ...
      <section>
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          ...
          {assignedTaskResults.length > 0 && (<button type="button" onClick={() => router.push(`/${orgSegment}/task-compass`)} className="shrink-0 py-2 px-4 rounded text-sm font-medium bg-[var(--color-primary)] hover:bg-[var(--color-primary-light)] text-white transition-colors">
              View all in Task Compass
            </button>)}
        </div>
        <div className="bg-[#262626]/50 border border-[#262626] rounded overflow-hidden">
          {assignedTaskResults.length > 0 ? (<div className="divide-y divide-[#262626]">
              {assignedTaskResults.map((result) => {
                ...
                return (<div key={result.taskId ?? taskTitle} className="p-6">
                    <h4 className="text-sm font-semibold text-white/90 mb-3 flex items-center gap-2">
                      {taskTitle}
                      {totalCount > 0 && (<span className="text-xs font-normal text-white/50">
                          ({files.length}{hasMore ? ` of ${totalCount}` : ''} files)
                        </span>)}
                    </h4>
                    <ul className="space-y-2">
                      {files.map((row, idx) => (<li key={idx} className="flex flex-wrap items-center gap-x-3 gap-y-1 py-2 px-3 rounded hover:bg-[#121215]/50 transition-colors">
            ...
            </div>) : (<div className="py-10 px-6 text-center">
              <p className="text-sm text-white/50 mb-4">No task analyses yet.</p>
              <button type="button" onClick={() => router.push(`/${orgSegment}/task-compass`)} className="py-2 px-4 rounded text-sm font-medium bg-[var(--color-primary)] hover:bg-[var(--color-primary-light)] text-white transition-colors">
