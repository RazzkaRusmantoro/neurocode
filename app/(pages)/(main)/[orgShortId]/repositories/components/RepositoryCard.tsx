...
export default function RepositoryCard({ id, name, urlName, url, orgShortId, source, addedAt, size, lastUpdate, description }: RepositoryCardProps) {
  ...
  return (
    <div
      onClick={handleClick}
      className="group relative border border-[#262626] rounded-lg p-6 hover:border-[#FF8D28]/50 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col bg-[#121215]"
      onMouseEnter={(e) => {
    ...
    >
      ...
      <h2 className="text-2xl font-bold text-white mb-3 group-hover:text-[#FF8D28] transition-colors duration-300">
        {name}
      </h2>

      {/* Repository URL Name Badge */}
      <div 
        className="w-fit inline-block pl-3 pr-7 py-1 rounded-lg text-sm font-medium mt-2 mb-3 bg-[#FF8D28]/20 text-[#FF8D28] border border-[#FF8D28]/30"
      >
        /{urlName}
      </div>
      ...
      {/* Hover Glow Effect */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#FF8D28]/0 to-[#FF8D28]/0 group-hover:from-[#FF8D28]/5 group-hover:to-transparent transition-all duration-300 pointer-events-none rounded-lg" />
    </div>
  );
}
