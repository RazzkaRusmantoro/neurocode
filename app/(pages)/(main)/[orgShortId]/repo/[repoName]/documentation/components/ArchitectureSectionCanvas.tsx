...
// Fixed node size so edge borderPoint math matches the visible box (no gap)
...
// ─── Section node (flowchart: label + role styling; description shown in panel on click) ─
...
// ─── Smart edge: border connection points + class diagram arrow (same as UML page) ─

/** Point where the ray from (cx,cy) toward (tx,ty) hits the rect centered at (cx,cy) with size (w,h). */
...
/** Edge that draws line from (x1,y1) to target border, with directed association arrow at target (same as UML). */
function BorderEdgeInner({ id, data, style }: EdgeProps) {
  ...
  return (
    <g>
      <BaseEdge
        id={id}
        path={`M ${d.x1} ${d.y1} L ${base.x} ${base.y}`}
...
type EdgeDef = { id: string; source: string; target: string; relationship: 'directed_association' };
...
/** Stored diagram from doc generation (nodes + edges + per-node description and role) */
export interface SectionDiagramSpec {
  nodes: { id: string; label: string; description?: string; role?: string }[];
...
export interface ArchitectureSectionCanvasProps {
  sectionId: string;
  sectionTitle: string;
  /** Flowchart from doc generation; canvas only renders when present and has nodes */
  diagram?: SectionDiagramSpec | null;
}

/** Simple horizontal layout: node index i at (i * (NODE_WIDTH + NODE_GAP), 0) */
function buildNodesAndEdgesFromDiagram(
  sectionId: string,
  diagram: SectionDiagramSpec,
): { initialNodes: Node[]; edgeDefs: EdgeDef[] } {
...
export default function ArchitectureSectionCanvas({
  sectionId,
  sectionTitle,
  diagram,
}: ArchitectureSectionCanvasProps) {
  const useDiagram = diagram?.nodes?.length;

  const { initialNodes: diagramNodes, edgeDefs: diagramEdgeDefs } = useMemo(() => {
    if (!useDiagram || !diagram) return { initialNodes: [], edgeDefs: [] };
  ...
  const closePanel = useCallback(() => {
    ...
    if (clearSelectedNodeTimeoutRef.current) clearTimeout(clearSelectedNodeTimeoutRef.current);
  ...
  const edges: Edge[] = useMemo(
    () => computeSmartEdges(nodes, edgeDefs),
    [nodes, edgeDefs]
  );

  return (
    <div
      className="rounded-lg overflow-hidden border border-white/10 bg-[#1a1a1d] relative"
    ...
    >
      ...
      {/* Animated description panel on node click */}
      <div
        className="absolute inset-0 pointer-events-none flex justify-end items-stretch"
        style={{ zIndex: 20 }}
      >
        <div
          className="pointer-events-auto overflow-hidden flex flex-col bg-[#1e1e21] border-l border-white/15 shadow-xl custom-scrollbar"
          style={{
            width: 320,
            maxWidth: '90%',
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 flex-shrink-0">
            ...
            <button
              type="button"
              onClick={closePanel}
              className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 transition-colors"
              aria-label="Close"
            >
      ...
      {panelVisible && (
        <button
          type="button"
          className="absolute inset-0 bg-black/40 pointer-events-auto z-10 transition-opacity duration-200"
          onClick={closePanel}
