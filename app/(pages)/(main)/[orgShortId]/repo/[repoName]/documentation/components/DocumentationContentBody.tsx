        ...
        <thead>
          <tr className="border-b border-white/20 bg-white/[0.06]">
            {headerRow.map((cell, ci) => (
              <th key={ci} className={`${cellPadding} text-left font-semibold text-white/95`}>{cell}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {bodyRows.map((row, ri) => (
            <tr key={ri} className={`border-b border-white/10 last:border-b-0 ${ri % 2 === 0 ? 'bg-transparent' : 'bg-white/[0.02]'}`}>
              {row.map((cell, ci) => (
                <td key={ci} className={`${cellPadding} text-left text-white/85 align-top`}>{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
          ...
          {sections.map((section, index) => (
            <div key={section.id} id={`section-${section.id}`} className="scroll-mt-6">
              ...
              <div className="prose prose-invert max-w-none mb-6 space-y-2">
                {parseDescription(section.description || '', onCodeRefClick).map((seg, i) =>
                  seg.type === 'code' ? (
                    <CodeSnippet key={i} code={seg.content} language={seg.language} />
                  ) : seg.type === 'table' ? (
                    <DocTable key={i} rows={seg.rows} />
                  ) : (
                    <p key={i} className="text-white leading-relaxed whitespace-pre-wrap">{seg.content}</p>
                  )
                )}
              </div>
              {section.subsections?.map((subsection) => (
                <div key={subsection.id} id={`subsection-${subsection.id}`} className="ml-6 mt-8 space-y-8 scroll-mt-6">
                  ...
                  <div className="prose prose-invert max-w-none mb-4 space-y-2">
                    {parseDescription(subsection.description || '', onCodeRefClick).map((seg, i) =>
                      seg.type === 'code' ? (
                        <CodeSnippet key={i} code={seg.content} language={seg.language} />
                      ) : seg.type === 'table' ? (
                        <DocTable key={i} rows={seg.rows} />
            ...
            {codeRefs.map((ref, idx) => {
              ...
              return (
                <div key={idx} id={`code-ref-${refId}`} className="scroll-mt-6">
                  ...
                  {refDetail?.signature && (
                    <div className="text-white/70 text-sm font-mono mb-4">{refDetail.signature}</div>
                  )}
                  {refDescription && (
                    <div className="prose prose-invert max-w-none mb-4 space-y-2">
                      {parseDescription(refDescription, onCodeRefClick).map((seg, i) =>
                        seg.type === 'code' ? (
                          <CodeSnippet key={i} code={seg.content} language={seg.language} />
                        ) : seg.type === 'table' ? (
                          <DocTable key={i} rows={seg.rows} />
