...
/**
 * GET /api/organizations/[orgShortId]/onboarding/generated-paths
 * Returns pathIds and pathSlugs for paths that have been generated (have an onboarding_path_doc record).
 * Used to show "Start learning" / "Continue learning" only when we have generated path docs.
 */
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    const { pathIds, pathSlugs } = await getGeneratedPathSlugsByOrganization(
    ...
    return NextResponse.json(
      { error: 'Failed to load generated paths' },
      { status: 500 }
    );
  }
}
