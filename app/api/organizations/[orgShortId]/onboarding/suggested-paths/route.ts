...
/**
 * GET /api/organizations/[orgShortId]/onboarding/suggested-paths
 * Returns cached suggested learning paths for the org (from DB). No LLM call.
 */
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to load suggested paths' },
      { status: 500 }
    );
  }
}
