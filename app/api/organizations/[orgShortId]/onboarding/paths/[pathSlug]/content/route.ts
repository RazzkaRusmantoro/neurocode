...
type RouteContext = {
  params: Promise<{ orgShortId: string; pathSlug: string }> | { orgShortId: string; pathSlug: string };
};

/**
 * GET /api/organizations/[orgShortId]/onboarding/paths/[pathSlug]/content
 * Returns generated documentation content for an onboarding path (from S3).
 * 404 if path has no generated doc or status is not completed.
 */
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: 'Failed to fetch path content from storage' },
    ...
    if (!s3Data.success || !s3Data.content) {
      return NextResponse.json(
        { error: 'Path content not available' },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to load path content' },
      { status: 500 }
    );
  }
}
